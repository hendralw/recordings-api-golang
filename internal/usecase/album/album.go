...
func (usecase *albumUsecase) Get(ctx context.Context, id uuid.UUID) (*entity.Album, error) {
	// Get from cache
...
// It will call the function GetAllAlbum in album repository
func (usecase *albumUsecase) GetAllAlbumByArtistID(ctx context.Context, id uuid.UUID) ([]entity.Album, error) {
	var albums []entity.Album

	// Get from db
	albums, err := usecase.albumRepository.GetAllAlbumByArtistID(ctx, id)
	if err != nil {
		return albums, err
	}
...
func (usecase *albumUsecase) Update(ctx context.Context, album entity.Album) (entity.Album, error) {
	...
	// Find new album
	updatedAlbum, err = usecase.albumRepository.Get(ctx, album.AlbumID)
	if err != nil {
		return *updatedAlbum, err
	}
	...
	// Set to specific cache
	if err = usecase.albumRepository.SetAlbumCache(ctx, album.AlbumID, *updatedAlbum); err != nil {
		return *updatedAlbum, err
	}
...
func (usecase *albumUsecase) Delete(ctx context.Context, id uuid.UUID) error {
	// Delete from db
